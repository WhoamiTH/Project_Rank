...
def calculate_knn(percentage, k, selected_threshold, noisy_threshold):
    ...
    knn_list = handle_data.create_knn_list(training_distance_matric, k)
    knn_list_name = str(percentage) + '_' + str(k) + '_knn_list.npz'
    save_by_npz(knn_list, knn_list_name)

    knn_label_list = handle_data.transform_knn_list_to_label(knn_list, label)
    knn_label_list_name = str(percentage) + '_' + str(k) + '_knn_label_list.npz'
    save_by_npz(knn_label_list, knn_label_list_name)

    final_knn_list = handle_data.eliminate_noisy(knn_label_list, selected_threshold, noisy_threshold, train_label)
